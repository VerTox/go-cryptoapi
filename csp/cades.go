//go:build linux && amd64 && cades

// Поддержка CAdES через libcades спрятана за тегом сборки `cades`, потому что
// она требует отдельно устанавливаемого и отдельно лицензируемого КриптоПро
// ЭЦП SDK (пакет cprocsp-pki-cades-64: заголовки <pki/cades.h> и libcades.so).
// Потребителям, которым нужна только проверка обычной открепленной PKCS#7 —
// см. VerifyDetachedCMS в cms.go — этот SDK не нужен, и заставлять их его
// ставить только ради компиляции неоправданно.
//
//	go build -tags cades ./...
package csp
